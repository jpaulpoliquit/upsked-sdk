/**
 * UP Baguio (UPB) AMIS course extractor (runs in browser on your campus AMIS origin, e.g. https://amis.upb.edu.ph)
 *
 ...
 *
 * INSTRUCTIONS:
 * 1. Log in at your AMIS base URL (e.g. https://amis.upb.edu.ph/)
 * 2. DevTools → Console, paste this file, Enter
 * 3. If prompted, copy `x-session-id` from Network → any request to api-amis.* → Headers
 */

(async function extractUpbAmisClasses() {
  console.log("Starting UPB AMIS course extraction...");

  try {
    const API_BASE = "https://api-amis.upb.edu.ph";
    ...
    if (!sessionId) {
      sessionId = window.prompt(
        "Paste x-session-id from DevTools → Network → request to api-amis.* → Request headers",
      );
    }
    if (!sessionId || !sessionId.trim()) {
      throw new Error(
        "No x-session-id: open Network tab while using AMIS, copy header from any API call.",
      );
    }
    ...
    console.log(
      `Active term: ${activeTermId} (${termsData.activeTerm.term} ${termsData.activeTerm.ay})`,
    );
    ...
    downloadAnchorNode.setAttribute("download", `upb-amis-classes-${activeTermId}.json`);
