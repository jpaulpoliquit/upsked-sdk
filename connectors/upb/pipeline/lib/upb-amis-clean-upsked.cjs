 ...
 *
 * Semester id is not in the payload; pass --semester-id (e.g. upb-2026-1).
 */
...
function instructorFromClass(cls) {
  ...
  const names = cls.faculties
    .map((f) => {
      ...
      const name =
        u?.formatted_name ||
        (u?.last_name && u?.first_name ? `${u.last_name}, ${u.first_name}` : null);
...
function* iterScheduleBlocks(cls) {
  const roomFallback =
    cls.facility_id && String(cls.facility_id).trim() ? String(cls.facility_id).trim() : null;
...
function mergePaginatedAmisCourses(filePaths) {
  ...
  parsed.sort((a, b) => (a.courses?.current_page || 0) - (b.courses?.current_page || 0));
  const seen = new Map();
...
/**
 * @param {object} params
 * @param {object} params.merged - parsed sample-amis-classes-merged.json
 * @param {string} params.semesterId - e.g. upb-2026-1
 ...
 */
function buildUpbUpskedArtifacts({ merged, semesterId, sourceLabel, mergedCourseCatalogRows }) {
  ...
  if (Array.isArray(mergedCourseCatalogRows) && mergedCourseCatalogRows.length > 0) {
    for (const row of mergedCourseCatalogRows) {
      deptCodes.add(row.subject || row.acad_org || "UPB");
    }
    for (const cls of classes) {
      deptCodes.add(cls.course?.subject || cls.course?.acad_org || cls.acad_org || "UPB");
    }
    ...
    for (const cls of classes) {
      ...
      deptCodes.add(c?.acad_org || cls.acad_org || "UPB");
  ...
  const departments = [...deptCodes].sort().map((code) => ({ code, name: code }));
  ...
  for (const cls of classes) {
    ...
    remarksParts.push(
      `Max: ${cls.max_class_size} | Enrolled: ${cls.active_class_size} | ${cls.status || ""}`,
    );
  ...
  const meta = {
    source: sourceLabel || "upb-amis-api",
    ...
    courseCatalogRows: Array.isArray(mergedCourseCatalogRows)
      ? mergedCourseCatalogRows.length
      : null,
...
function writeBundle(outDir, bundle) {
  ...
  fs.writeFileSync(
    path.join(outDir, "departments.json"),
    JSON.stringify(bundle.departments, null, 2),
  );
  fs.writeFileSync(
    path.join(outDir, "sections-upsked.json"),
    JSON.stringify(bundle.sections, null, 2),
  );
  fs.writeFileSync(
    path.join(outDir, "schedules-upsked.json"),
