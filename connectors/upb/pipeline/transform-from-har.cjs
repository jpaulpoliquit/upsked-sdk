...
const harPath = path.resolve(
  process.argv[2] || path.join(REPO_ROOT, "..", "docs", "amis-sample.har"),
);

if (!fs.existsSync(harPath)) {
  console.error(`HAR not found: ${harPath}`);
  console.error("Pass path as first argument or place docs/amis-sample.har next to the repo root.");
  process.exit(1);
}

const harData = JSON.parse(fs.readFileSync(harPath, "utf8"));
const classesRequest = harData.log.entries.find((e) =>
  e.request.url.includes("/api/students/classes"),
);
if (!classesRequest) {
...
const outDir = path.join(REPO_ROOT, "fixtures", "upb");
fs.mkdirSync(outDir, { recursive: true });
const outputPath = path.join(outDir, "upb-sections-raw.json");
