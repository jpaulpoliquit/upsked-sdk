 ...
 *   node pipeline/clean-for-upsked.cjs --input ./fixtures/uplb/sample-amis-classes-merged.json --out ./fixtures/uplb/sample-upsked-bundle --semester-id uplb-2026-1
 */
const fs = require('fs');
...
const {
  ...
  buildProgramsUpsked,
} = require('./lib/uplb-amis-clean-upsked.cjs');

const REPO_ROOT = path.join(__dirname, '..', '..', '..');

function parseArgs(argv) {
  const out = {
    input: path.join(REPO_ROOT, 'fixtures', 'uplb', 'sample-amis-classes-merged.json'),
    outDir: path.join(REPO_ROOT, 'fixtures', 'uplb', 'sample-upsked-bundle'),
    semesterId: process.env.UPLB_SEMESTER_ID || 'uplb-2026-1',
    coursePagePaths: [],
    programsPath: null,
  };
...
function main() {
  ...
  const bundle = buildUplbUpskedArtifacts({
    merged,
    semesterId,
  ...
  const programs =
    programsPath != null ? buildProgramsUpsked(JSON.parse(fs.readFileSync(programsPath, 'utf8'))) : undefined;
